...
int main(void)
{
    ...
    while (clock_time < 200)
    {
        ...
        while (temp.arrival_time == clock_time)
        {
            ...
            fgetc(inf);
            quicksort(job_queue, current_job+1, queue_size-1);
        }
...
void quicksort(JOB* arr, int first, int last)
{
    ...
    int pivot_a = (i + j) / 2;
    JOB pivot = arr[pivot_a];
    
    while (i <= j)
    {
        while (arr[i].priority_level > pivot.priority_level)
            i++;
        while (arr[j].priority_level < pivot.priority_level)
            j--;
        if (i <= j)
        {
            // swap array values
